...
with tab_corr:
    ...
    pool = picked if len(picked) >= 2 else candidates[:MAX_LIST]
    st.caption("Pick from the series you selected on the left, or search here."
               if len(picked) >= 2 else
               "Select two or more series on the left to narrow this list.")
    c1, c2 = st.columns(2)
    x = c1.selectbox("x", pool, index=0, format_func=short, key="cx")
    y = c2.selectbox("y", pool, index=min(1, len(pool) - 1),
                     format_func=short, key="cy")
    f3, r, rs, n = fig_scatter(x, y)
    st.pyplot(f3)
    st.session_state["fig_sc"] = png(f3)
    st.session_state["corr"] = {"x": x, "y": y, "pearson": r, "spearman": rs, "n": n}
    if abs(r - rs) > 0.15:
        st.warning(f"Pearson ({r:.2f}) and Spearman ({rs:.2f}) differ by "
                   f"{abs(r - rs):.2f}: the relationship is not linear, or a few "
                   f"extreme points dominate.")
    with st.form("note_corr", clear_on_submit=True):
        ...
        if st.form_submit_button("Save note") and txt.strip():
            save_note(f"{short(x)} vs {short(y)}", who, txt)
            st.success("saved")
